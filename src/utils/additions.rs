...
impl AdditionDef {
    ...
    pub unsafe fn parse_unchecked(line: &[u8]) -> AdditionDef {
        ...
        let code = from_utf8_unchecked(&line[4..7]);

        AdditionDef::new(code, start_byte, end_byte)
    }
}

pub type AdditionsMap = HashMap<String, Vec<u8>>;
...
mod tests {
    ...
    #[test]
    fn parse_example_1() {
        let additions_defs = vec![AdditionDef::new("HDT", 8, 12)];
        let additions = additions_defs.parse(b"K12345600090").unwrap();
        assert_eq!(additions.len(), 1);
        assert_eq!(additions.get("HDT"), Some(&b"00090".to_vec()));
    }
    ...
    fn parse_example_2() {
        let additions_defs = vec![
            ...
            AdditionDef::new("SIU", 39, 40),
        ];
        let additions = additions_defs.parse(b"B0818265049456N00610940EA011730132000308").unwrap();
        assert_eq!(additions.len(), 2);
        assert_eq!(additions.get("FXA"), Some(&b"003".to_vec()));
        assert_eq!(additions.get("SIU"), Some(&b"08".to_vec()));
    }
